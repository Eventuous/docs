...
Our [`ApplicationService`](../../application/app-service.md) uses the `AggregateStore` in its command-handling flow.
...
The connector is running continuously, and it copies all the events from the hot store to the archive store. It subscribes to the hot store using a catch-up subscription to the global log, and copies all the events from there to the archive. In this example, we use the [Elastic Connector](../../infra/elastic) to connect the hot store to the archive store.
