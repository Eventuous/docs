...
Check the diagram below to see the whole picture.

- The beating heart of the application is the domain model. It contains critical business-oriented functionality, encapsulated in the domain objects. The domain model is the only place where the business logic is implemented.
- Command services receive commands from the outside world and dispatch them to the domain model. They are responsible for the validation of the commands and calling the domain object functions, as well as dealing with persistence.
- New events are stored in the event store. There, we _must_ have a database that is able to push new events to real-time subscriptions.
- Stored events are pushed by the event store to subscriptions.
  - Subscriptions could produce integration (public) events. In Eventuous, the Gateway component is responsible for that.
  - Another type of subscription could project events to other databases for queries or analytics. Eventuous has support for projecting events to MongoDB.
...
- Using some kind of bus to propagate [domain events](../../domain/domain-events) to read models
...
[Publishing](../../producers) events to a message broker from a [subscription](../../subscriptions) is an entirely legit scenario. Do not build reporting models by consuming from a broker unless it's an event log-based broker like Apache Kafka, Apache Pulsar or Azure Event Hub. Prefer projecting events by subscribing directly to your event store.
